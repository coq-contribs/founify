...
intros; auto with arith. (* apply le_n_S ; apply le_O_n *)
(* Case2 l=(Consv x l0) and (IS_IN_LV x l0) *)
...
elim y0; apply Unif_succeed with (fun x : var => V x);
 ...
 auto with arith; intros.
absurd (V y1 = V y1); auto with arith.
...
replace t with (x y1); auto with arith.
symmetry  in |- *; auto with arith.
absurd (V y1 = x y1); auto with arith.
unfold min_unif, unif in |- *; intros.
...
apply Unif_succeed with V;
 ...
 auto with arith.
intros.
absurd (V y0 = V y0); auto with arith.
...
elim (IS_IN_LV_IS_IN (ConsArg t2 t4) y0 H1); auto with arith.
...
apply (H1 q2); elim b0; auto with arith.
   ...
   (***************************************************************)
elim (H0 q1); elim b; auto with arith; intros. (*apply UnifConsArgfail1*)
elim (ident_or_notS (ConsArg q q1) f o); intros.
elim a; intros x H6.
