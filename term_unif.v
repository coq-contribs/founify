...
intros; auto with v62. (* Apply le_n_S ; Apply le_O_n *)
(* Case2 l=(Consv x l0) and (IS_IN_LV x l0) *)
...
(*Case t=(Root f y) *)
intros; simpl in |- *; apply H; apply closRoot with (1 := H0).
(*Case t=(ConsArg y y0) *)
...
Inductive Unification (t1 t2 : quasiterm) : Set :=
  | Unif_succeed :
      ...
      (*------------ Verifications on the terms:end --------------*)
       -> Unification t1 t2
  | Unif_fail :
      (forall f : quasisubst, Subst f t1 <> Subst f t2) -> Unification t1 t2.
...
intros; elim H; unfold idempotent, over, under, min_unif, unif in |- *;
 intros.
...
intros; elim (IS_IN_decS n t); intro y.
elim (quasiterm_eq_decS (V n) t); intro y0.
(*Case (IS_IN n t) and <quasiterm>(V n)=t*)
elim y0; apply Unif_succeed with (fun x : var => V x);
 unfold unif, idempotent, over, under in |- *; simpl in |- *; 
 auto with v62; intros.
absurd (V y1 = V y1); auto with v62.
...
elim (var_eq_decP n y1); intros.
replace t with (x y1); auto with v62.
symmetry  in |- *; auto with v62.
absurd (V y1 = x y1); auto with v62.
unfold min_unif, unif in |- *; intros.
...
intros; elim (fun_eq_decS l f); intro y.
elim y; apply Unif_succeed with V;
 unfold unif, idempotent, over, under in |- *; simpl in |- *; 
 auto with v62.
...
intros; elim (fun_eq_decS l1 l2); intro y.
elim y; elim H; unfold unif, idempotent, over, under in |- *; intros.
...
intros; elim (quasiterm_eq_decS t1 t2); intro y.
elim y.
apply Unif_succeed with V;
 unfold unif, idempotent, over, under, min_unif in |- *; 
 auto with v62.
intros.
absurd (V y0 = V y0); auto with v62.
...
intros; elim (le_decS n0 n); intro y.
apply (le_S_eqS n0 n y).
...
elim (IS_IN_LV_IS_IN (ConsArg t2 t4) y0 H1); auto with v62.
...
elim H1; unfold unif, over, under, idempotent, min_unif in |- *; intros.
(*Success*)
...
elim (DIFFELNB_ConsArg_ConsArg13_le q q0 q1 q2 (S p) H4 p0 H5); intros.
   ...
   (***************************************************************)
elim (H p0 (le_S_n p0 p a) q q1 H5); intros.
2: auto with v62.
...
apply (H1 q2); elim b0; auto with v62.
   ...
   (***************************************************************)
elim (H0 q1); elim b; auto with v62; intros. (*Apply UnifConsArgfail1*)
elim (ident_or_notS (ConsArg q q1) f o); intros.
elim a; intros x H6.
...
elim (DIFFELNBor (list_var (ConsArg q0 q2))); intros p1 H6.
elim (DIFFELNB_ConsArg_ConsArg24_le q q0 q1 q2 (S p) H4 p1 H6); intros.
apply (H p1); auto with v62. (*Apply Apply le_S_n*)
